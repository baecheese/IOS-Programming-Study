...
- (void)viewDidLoad {
    ...
    // 데이터 재 로딩
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reloadDataWithAFNetwork) name:ImageUploadDidSuccessNotification object:nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // URL Array 프로퍼티에 서버에서 내려받은 image URL 넣기
...
{
    [[RequstObject shareInstance] reloadDataWithAFNetwork];
    ...
    UIAlertAction *okButton = [UIAlertAction actionWithTitle:@"확인" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if (1 > alert.textFields.firstObject.text.length) {
            ...
            if (useProperty == NO) {
                ...
                // 테이블뷰 재로딩 -> 데이터 다시 받아오고 재로딩
                //[self requestImageList];
                [self reloadDataWithAFNetwork];
//                [self requestImageList];
            }
            else {
                ...
                /* image upload 업로드 - AFNetwork */
                
                [[RequstObject shareInstance] uploadTaskForMutipartWithAFNetwork:self.selectedImage title:inputText];
                
        
                //[[RequstObject shareInstance] uploadImage:self.selectedImage title:inputText];
